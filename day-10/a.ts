...
export async function day10a(data: string[]) {
  ...
  function getPeaksReachable(
    ...
    visitedPositions.add(positionString);
    ...
    for (const [dx, dy] of change) {
      ...
      peaksReachable += getPeaksReachable(nextPosition, visitedPositions);
    }
  ...
  for (const trailheadPosition of trailheadPositions) {
    const peaksReachable = getPeaksReachable(trailheadPosition, new Set());
